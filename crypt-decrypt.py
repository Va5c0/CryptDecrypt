...
def main():
    ...
    argp.add_argument('-F', '--file', dest='file',
                      help='File with hashes to decrypt')
    ...
    if args.encrypt and not args.all and not args.decrypt and not args.identify:  # ENCRYPTER
        ...
        if args.online and not args.offline:  # Online
            ...
            print(" [+]" + colors.INFO + " Type: " + colors.ENDC + alg)
            if alg == "md5":
                md5OffDecryptFile(hashs, args.wordlist)
            elif alg == "sha1":
                sha1OffDecryptFile(hashs, args.wordlist)
            elif alg == "sha224":
                sha224OffDecryptFile(hashs, args.wordlist)
            elif alg == "sha256":
                sha256OffDecryptFile(hashs, args.wordlist)
            elif alg == "sha384":
                sha384OffDecryptFile(hashs, args.wordlist)
            elif alg == "sha512":
                sha512OffDecryptFile(hashs, args.wordlist)
            elif alg == "ntlm":
                print(colors.FAIL + colors.BOLD + "\n[!] Hash type not supported in this mode" + colors.ENDC + "\n")
            elif alg == "lm":
        ...
        with open(args.file, 'r') as myfile:
            if args.online:  # Online
                ...
                for line in myfile.readlines():
                    ...
                    if alg == "md5":
                        md5OffDecryptFile(line, args.wordlist)
                    elif alg == "sha1":
                        sha1OffDecryptFile(line, args.wordlist)
                    elif alg == "sha224":
                        sha224OffDecryptFile(line, args.wordlist)
                    elif alg == "sha256":
                        sha256OffDecryptFile(line, args.wordlist)
                    elif alg == "sha384":
                        sha384OffDecryptFile(line, args.wordlist)
                    elif alg == "sha512":
                        sha512OffDecryptFile(line, args.wordlist)
                    elif alg == "ntlm":
                        print(colors.FAIL + colors.BOLD + "\n[!] Hash type not supported in this mode" + colors.ENDC + "\n")
                    elif alg == "lm":
